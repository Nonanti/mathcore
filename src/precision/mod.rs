...
use num_traits::{Zero, One, ToPrimitive, FromPrimitive, Signed};
...
impl PrecisionNumber {
        ...
        match (self, exponent) {
            (PrecisionNumber::Integer(base), PrecisionNumber::Integer(exp)) => {
                if let Some(exp_u32) = exp.to_u32() {
                    ...
                    if let Some(exp_u32) = positive_exp.to_u32() {
                        ...
                        Err(MathError::InvalidOperation("Exponent too large".to_string()))
                    }
                } else {
                    Err(MathError::InvalidOperation("Exponent too large".to_string()))
                }
            ...
            _ => {
                if let (Some(base_f64), Some(exp_f64)) = (self.to_f64(), exponent.to_f64()) {
                    ...
                    Err(MathError::InvalidOperation("Cannot compute power with these operands".to_string()))
                }
            }
        }
    }
    ...
    pub fn factorial(&self) -> Result<PrecisionNumber, MathError> {
        match self {
            ...
            _ => Err(MathError::InvalidOperation("Factorial requires non-negative integer".to_string()))
        }
    }
    ...
    pub fn to_f64(&self) -> Option<f64> {
        match self {
            ...
            PrecisionNumber::Exact(r) => {
                r.numer().to_f64().and_then(|n| {
                    r.denom().to_f64().map(|d| n / d)
                })
            }
        }
    }
    ...
    pub fn sqrt(&self) -> Result<PrecisionNumber, MathError> {
        match self {
            PrecisionNumber::Integer(n) if !n.is_negative() => {
                let n_f64 = n.to_f64().ok_or_else(|| MathError::Overflow)?;
                let sqrt_val = n_f64.sqrt();
                
                if sqrt_val.fract() == 0.0 {
                    Ok(PrecisionNumber::Integer(BigInt::from_f64(sqrt_val).unwrap()))
                } else {
                    Ok(PrecisionNumber::Float(sqrt_val))
                }
            }
            PrecisionNumber::Float(f) if *f >= 0.0 => {
                Ok(PrecisionNumber::Float(f.sqrt()))
            }
            PrecisionNumber::Exact(r) if !r.is_negative() => {
                let n_f64 = r.to_f64().ok_or_else(|| MathError::Overflow)?;
                Ok(PrecisionNumber::Float(n_f64.sqrt()))
            }
            _ => Err(MathError::InvalidOperation("Cannot take square root of negative number".to_string()))
        }
    }
}
