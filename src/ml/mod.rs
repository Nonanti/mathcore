use crate::types::{Expr, BinaryOp, MathError};
...
impl Optimization {
        ...
        for _ in 0..iterations {
            ...
            for (var_name, grad_expr) in var_names.iter().zip(gradients.iter()) {
                let grad_value = match engine.evaluate_with_vars(grad_expr, &params)? {
                    Expr::Number(n) => n,
                    _ => return Err(MathError::InvalidOperation("Gradient must be numeric".to_string()))
                };
    ...
    pub fn automatic_differentiation(expr: &Expr, var: &str, value: f64) -> Result<(f64, f64), MathError> {
        let engine = Engine::new();
        ...
        let function_value = match engine.evaluate_with_vars(expr, &vars)? {
            Expr::Number(n) => n,
            _ => return Err(MathError::InvalidOperation("Expression must evaluate to number".to_string()))
        };
        
        let derivative = Calculus::differentiate(expr, var)?;
        let derivative_value = match engine.evaluate_with_vars(&derivative, &vars)? {
            Expr::Number(n) => n,
            _ => return Err(MathError::InvalidOperation("Derivative must evaluate to number".to_string()))
        };
        ...
        for (i, func) in functions.iter().enumerate() {
            for (j, var) in vars.iter().enumerate() {
                ...
                let value = match engine.evaluate_with_vars(&partial, point)? {
                    Expr::Number(n) => n,
                    _ => return Err(MathError::InvalidOperation("Jacobian entry must be numeric".to_string()))
                };
                jacobian[(i, j)] = value;
            }
        }
    ...
    pub fn taylor_series(expr: &Expr, var: &str, center: f64, order: usize) -> Result<Expr, MathError> {
        let engine = Engine::new();
    ...
    pub fn optimize_newton(
        ...
        for _ in 0..max_iterations {
            ...
            let f_prime = match engine.evaluate_with_vars(&first_deriv, &vars)? {
                Expr::Number(n) => n,
                _ => return Err(MathError::InvalidOperation("Derivative must be numeric".to_string()))
            };
            ...
            let f_double_prime = match engine.evaluate_with_vars(&second_deriv, &vars)? {
                Expr::Number(n) => n,
                _ => return Err(MathError::InvalidOperation("Second derivative must be numeric".to_string()))
            };
            
            if f_double_prime.abs() < 1e-10 {
                return Err(MathError::InvalidOperation("Second derivative too small".to_string()));
            }
