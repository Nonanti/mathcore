...
impl Solver {
    ...
    fn normalize_equation(equation: &Expr) -> Result<Expr, MathError> {
        match equation {
            Expr::Binary { op: BinaryOp::Subtract, .. } => Ok(equation.clone()),
            _ => Ok(Expr::Binary {
    ...
    fn solve_numerical(equation: &Expr, var: &str) -> Result<Vec<Expr>, MathError> {
        ...
        let search_points = vec![-100.0, -10.0, -1.0, 0.0, 1.0, 10.0, 100.0];
    ...
    fn newton_raphson(equation: &Expr, var: &str, initial: f64, engine: &Engine) -> Result<Option<Expr>, MathError> {
    ...
    fn extract_linear_coefficients(equation: &Expr, var: &str) -> Result<(f64, f64), MathError> {
        ...
        let b = match engine.evaluate(&subst_zero)? {
            Expr::Number(n) => n,
            _ => return Err(MathError::SolverError("Cannot extract constant term".to_string())),
        };
        
        let subst_one = engine.substitute(equation, var, &Expr::one())?;
        let val_at_one = match engine.evaluate(&subst_one)? {
            Expr::Number(n) => n,
            _ => return Err(MathError::SolverError("Cannot extract linear coefficient".to_string())),
        };
    ...
    fn extract_quadratic_coefficients(equation: &Expr, var: &str) -> Result<(f64, f64, f64), MathError> {
        let engine = Engine::new();
