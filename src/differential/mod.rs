...
pub enum BoundaryCondition {
    InitialValue { t0: f64, y0: Vec<f64> },
    BoundaryValue { ta: f64, tb: f64, ya: Vec<f64>, yb: Vec<f64> },
}
...
impl DifferentialEquations {
    ...
    pub fn solve_ode_first_order(
        ...
        for _ in 0..steps {
            ...
            let k1 = match engine.evaluate_with_vars(f, &vars)? {
                Expr::Number(n) => n,
                _ => return Err(MathError::InvalidOperation("ODE function must return number".to_string()))
            };
    ...
    pub fn solve_ode_system(
        ...
        if functions.len() != y_vars.len() || functions.len() != initial_conditions.1.len() {
            return Err(MathError::InvalidOperation("Dimension mismatch in ODE system".to_string()));
        }
        ...
        for _ in 0..steps {
            ...
            for _ in 0..10 {
                ...
                if newton_denominator.abs() < 1e-10 {
                    return Err(MathError::InvalidOperation("Newton iteration failed".to_string()));
                }
    ...
    pub fn solve_linear_constant_coeff(
        ...
        if coefficients.is_empty() || initial_conditions.is_empty() {
            return Err(MathError::InvalidOperation("Empty coefficients or initial conditions".to_string()));
        }
        
        let n = coefficients.len() - 1;
        if initial_conditions.len() != n {
            return Err(MathError::InvalidOperation("Initial conditions count mismatch".to_string()));
        }
...
impl PDESolver {
    ...
    pub fn solve_heat_equation(
        ...
        if stability > 0.5 {
            return Err(MathError::InvalidOperation(
                format!("Unstable: α*dt/dx² = {} > 0.5", stability)
            ));
        }
    ...
    pub fn solve_wave_equation(
        ...
        if courant > 1.0 {
            return Err(MathError::InvalidOperation(
                format!("CFL condition violated: c*dt/dx = {} > 1", courant)
            ));
        }
