        ...
        match expression(input) {
            Ok((remaining, expr)) => {
                if remaining.trim().is_empty() {
                    ...
                    Err(MathError::ParseError(format!("Unexpected input: '{}'", remaining)))
                }
            }
...
fn additive(input: &str) -> IResult<&str, Expr> {
    ...
    Ok((input, operations.into_iter().fold(initial, |acc, (op, expr)| {
        let op = match op {
            '+' => BinaryOp::Add,
            '-' => BinaryOp::Subtract,
            _ => unreachable!(),
...
mod tests {
    ...
    fn test_parse_binary_ops() {
        ...
        assert!(matches!(expr, Expr::Binary { op: BinaryOp::Power, .. }));
    }
