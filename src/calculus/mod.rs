...
use crate::types::{Expr, BinaryOp, UnaryOp, MathError};
...
impl Calculus {
        ...
        match expr {
            ...
            Expr::Binary { op, left, right } => {
                match op {
                    ...
                    _ => Err(MathError::InvalidOperation(format!("Cannot differentiate {:?} operation", op)))
                }
            }
            ...
            Expr::Function { name, args } => {
                Self::diff_function(name, args, var)
            }
            
            Expr::Derivative { expr: inner, var: d_var, order } => {
                if d_var == var {
                    Ok(Expr::Derivative {
                        expr: inner.clone(),
                        var: var.to_string(),
            ...
            _ => Err(MathError::InvalidOperation("Cannot differentiate this expression".to_string()))
        }
    }
    ...
    fn integrate_internal(expr: &Expr, var: &str) -> Result<Expr, MathError> {
        match expr {
            ...
            Expr::Binary { op, left, right } if matches!(op, BinaryOp::Add | BinaryOp::Subtract) => {
            ...
            Expr::Binary { op: BinaryOp::Multiply, left, right } => {
            ...
            Expr::Binary { op: BinaryOp::Power, left, right } => {
    ...
    pub fn numerical_integrate(expr: &Expr, var: &str, lower: f64, upper: f64, steps: usize) -> Result<f64, MathError> {
        ...
        for i in 0..steps {
            ...
            let y1 = match engine.evaluate_with_vars(expr, &vars)? {
                Expr::Number(n) => n,
                _ => return Err(MathError::InvalidOperation("Expected numeric result".to_string()))
            };
    ...
    fn simplify_basic(expr: &Expr) -> Expr {
        match expr {
            Expr::Binary { op, left, right } => {
                ...
                match (op, &left, &right) {
                    (BinaryOp::Add, e, other) | (BinaryOp::Add, other, e) if e.is_zero() => other.clone(),
                    (BinaryOp::Subtract, e, other) if other.is_zero() => e.clone(),
                    (BinaryOp::Subtract, e, other) if e.is_zero() => {
                        Expr::Unary {
                            op: UnaryOp::Negate,
                            expr: Box::new(other.clone()),
                        }
                    }
                    (BinaryOp::Multiply, e, _) | (BinaryOp::Multiply, _, e) if e.is_zero() => Expr::zero(),
                    (BinaryOp::Multiply, e, other) | (BinaryOp::Multiply, other, e) if e.is_one() => other.clone(),
