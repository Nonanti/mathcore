...
impl Limits {
    pub fn limit(expr: &Expr, var: &str, point: f64, direction: LimitDirection) -> Result<Expr, MathError> {
        ...
        if direction == LimitDirection::Both {
            ...
            if Self::expressions_equal(&left, &right, 1e-9) {
                ...
                return Err(MathError::InvalidOperation("Limit does not exist (left != right)".to_string()));
            }
        }
    ...
    fn numerical_limit(expr: &Expr, var: &str, point: f64, epsilon: f64) -> Result<Expr, MathError> {
        ...
        for test_point in test_points {
            ...
            match engine.evaluate_with_vars(expr, &vars) {
                ...
                Ok(Expr::Number(n)) if n.is_infinite() => {
                    return Ok(Expr::Symbol(if n.is_sign_positive() { "∞".to_string() } else { "-∞".to_string() }));
                },
                _ => {}
            }
        }
        ...
        if results.is_empty() {
            return Err(MathError::InvalidOperation("Cannot compute limit".to_string()));
        }
        
        let avg = results.iter().sum::<f64>() / results.len() as f64;
        let variance = results.iter().map(|x| (x - avg).powi(2)).sum::<f64>() / results.len() as f64;
        ...
        if variance < 1e-12 {
            ...
            if trend.abs() > 1e-6 {
                Ok(Expr::Symbol(if trend > 0.0 { "∞".to_string() } else { "-∞".to_string() }))
            } else {
                Ok(Expr::Number(avg))
            }
        }
    ...
    pub fn lhopital_rule(numerator: &Expr, denominator: &Expr, var: &str, point: f64) -> Result<Expr, MathError> {
