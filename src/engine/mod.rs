use crate::types::{Expr, BinaryOp, UnaryOp, Context, MathError};
...
impl Engine {
    ...
    pub fn evaluate_with_vars(&self, expr: &Expr, vars: &HashMap<String, f64>) -> Result<Expr, MathError> {
        ...
        match (left, right) {
            ...
            (Expr::Complex(l), Expr::Complex(r)) => {
                let result = match op {
                    ...
                    BinaryOp::Modulo => {
                        return Err(MathError::InvalidOperation("Modulo not defined for complex numbers".to_string()));
                    }
                };
        ...
        match (op, expr) {
            ...
            (UnaryOp::Factorial, Expr::Number(n)) => {
                if *n < 0.0 || n.fract() != 0.0 {
                    return Err(MathError::InvalidOperation("Factorial requires non-negative integer".to_string()));
                }
                let mut result = 1.0;
                let mut i = 2.0;
                while i <= *n {
    ...
    fn apply_function(&self, name: &str, args: &[Expr]) -> Result<Expr, MathError> {
        match name {
            "sin" => {
                if args.len() != 1 {
                    return Err(MathError::InvalidArgumentCount(name.to_string(), 1, args.len()));
                }
                match &args[0] {
                    Expr::Number(n) => Ok(Expr::Number(n.sin())),
                    Expr::Complex(c) => Ok(Expr::Complex(c.sin())),
                    _ => Ok(Expr::Function { name: name.to_string(), args: args.to_vec() })
                }
            }
            ...
            "log" => {
                if args.len() != 2 {
                    return Err(MathError::InvalidArgumentCount(name.to_string(), 2, args.len()));
                }
                match (&args[0], &args[1]) {
                    (Expr::Number(n), Expr::Number(base)) if *n > 0.0 && *base > 0.0 && *base != 1.0 => {
            ...
            "min" => {
                ...
                let nums: Result<Vec<f64>, _> = args.iter().map(|arg| {
                    match arg {
                        Expr::Number(n) => Ok(*n),
                        _ => Err(MathError::InvalidOperation("min requires numeric arguments".to_string()))
                    }
                }).collect();
                let nums = nums?;
            ...
            "max" => {
                ...
                let nums: Result<Vec<f64>, _> = args.iter().map(|arg| {
                    match arg {
                        Expr::Number(n) => Ok(*n),
                        _ => Err(MathError::InvalidOperation("max requires numeric arguments".to_string()))
                    }
                }).collect();
                let nums = nums?;
                Ok(Expr::Number(nums.into_iter().fold(f64::NEG_INFINITY, f64::max)))
            }
        ...
        match expr {
            ...
            Expr::Function { name, args } => {
                let new_args: Result<Vec<_>, _> = 
                    args.iter().map(|arg| self.substitute(arg, var, value)).collect();
                Ok(Expr::Function {
                    name: name.clone(),
                    args: new_args?,
                })
