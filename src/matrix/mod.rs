...
impl SymbolicMatrix {
    ...
    pub fn from_vec(data: Vec<Vec<f64>>) -> Result<Self, MathError> {
        ...
        if !data.iter().all(|row| row.len() == cols) {
            return Err(MathError::InvalidOperation("Inconsistent row lengths".to_string()));
        }
        
        let expr_data = data.into_iter()
            .map(|row| row.into_iter().map(Expr::Number).collect())
            .collect();
        
        Ok(SymbolicMatrix { rows, cols, data: expr_data })
    }
    ...
    pub fn add(&self, other: &SymbolicMatrix) -> Result<SymbolicMatrix, MathError> {
        if self.rows != other.rows || self.cols != other.cols {
            return Err(MathError::InvalidOperation("Matrix dimensions don't match".to_string()));
        }
    ...
    pub fn multiply(&self, other: &SymbolicMatrix) -> Result<SymbolicMatrix, MathError> {
        if self.cols != other.rows {
            return Err(MathError::InvalidOperation(
                format!("Cannot multiply {}x{} with {}x{}", self.rows, self.cols, other.rows, other.cols)
            ));
        }
    ...
    pub fn determinant(&self) -> Result<Expr, MathError> {
        if self.rows != self.cols {
            return Err(MathError::InvalidOperation("Determinant requires square matrix".to_string()));
        }
    ...
    pub fn trace(&self) -> Result<Expr, MathError> {
        if self.rows != self.cols {
            return Err(MathError::InvalidOperation("Trace requires square matrix".to_string()));
        }
...
impl LinearAlgebra {
    ...
    pub fn eigenvalues(matrix: &DMatrix<f64>) -> Result<Vec<num_complex::Complex64>, MathError> {
        if !matrix.is_square() {
            return Err(MathError::InvalidOperation("Eigenvalues require square matrix".to_string()));
        }
    ...
    pub fn svd(matrix: &DMatrix<f64>) -> Result<(DMatrix<f64>, DVector<f64>, DMatrix<f64>), MathError> {
        ...
        match (svd.u, svd.v_t) {
            ...
            _ => Err(MathError::InvalidOperation("SVD computation failed".to_string()))
        }
    }
    ...
    pub fn condition_number(matrix: &DMatrix<f64>) -> Result<f64, MathError> {
        if !matrix.is_square() {
            return Err(MathError::InvalidOperation("Condition number requires square matrix".to_string()));
        }
